...
async def lifespan(app: FastAPI):
    # Load off the event loop so /health answers during a cold start; /ready
    # gates traffic until the checkpoint is resident.
...
app = FastAPI(
    title=f"COMET scoring service ({MODEL_NAME})",
    version="0.1.0",
    summary="Serves any COMET checkpoint. src/mt (+ optional ref) in, scores out.",
    lifespan=lifespan,
)
...
@app.get("/ready", summary="Readiness — is the checkpoint resident")
...
async def score(req: ScoreRequest) -> ScoreResponse:
    ...
    # ref is dropped when absent so a reference-free checkpoint receives exactly
    # the {src, mt} dict its prepare_sample expects.
    rows = [
        {"src": r.src, "mt": r.mt} if r.ref is None else {"src": r.src, "mt": r.mt, "ref": r.ref}
        for r in req.rows
    ]
