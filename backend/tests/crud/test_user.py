...
def test_create_user_default_role(db: Session) -> None:
...
def test_get_user(db: Session) -> None:
    password = random_lower_string()
    username = random_email()
    user_in = UserCreate(email=username, password=password, role="admin")
    user = crud.create_user(session=db, user_create=user_in)
...
def test_update_user(db: Session) -> None:
    ...
    user_in_update = UserUpdate(password=new_password, role="admin")
...
def test_authenticate_user_with_bcrypt_upgrades_to_argon2(db: Session) -> None:
    ...
    user = User(email=email, hashed_password=bcrypt_hash, role_id=member_role.id)
    db.add(user)
    db.commit()
    db.refresh(user)
