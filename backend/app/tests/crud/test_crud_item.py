...
from app.models import ItemCreate, ItemUpdate, User, UserCreate
...
def test_get_items(db: Session) -> None:
    ...
    item1 = crud.create_item(
        session=db, item_in=ItemCreate(title=random_lower_string()), owner_id=owner.id
    )
...
def test_get_items_by_owner(db: Session) -> None:
    ...
    item1 = crud.create_item(
        session=db, item_in=ItemCreate(title=random_lower_string()), owner_id=owner1.id
    )
    item2 = crud.create_item(
        session=db, item_in=ItemCreate(title=random_lower_string()), owner_id=owner2.id
    )
